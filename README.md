...
> **Если на телефоне «не открывается»** — скорее всего, роутер выдал компьютеру новый IP, и сохранённая на телефоне ссылка вида `192.168.х.х` указывает в никуда. Поэтому запуск печатает второй, **постоянный** адрес вида `http://Имя-Компьютера.local:8934/` (он же показан под QR-кодом): этот адрес не меняется при переподключении к Wi-Fi, его и стоит сохранить в закладки на телефоне.
